...
class SongInfo:
    ...
    async def from_ytdl(cls, request, requester, channel, loop=None):
        ...
        if processed_info is None:
            await ctx.send(f'Could not retrieve info from {request}')
...
class GuildMusicState:
    ...
    async def play_next_song(self, song=None, error=None):
        ...
        if self.playlist.empty():
            await ctx.send("There are currently no songs playing, use `?play <str>` to play one.")
        else:
            ...
            e = discord.Embed(description=f'Now playing {next_song_info}')
            await next_song_info.channel.send(embed=e)
...
class Music:
    ...
    async def status(self, ctx):
        ...
        if ctx.music_state.is_playing():
            ...
            await ctx.send(f'**:package: | Currently playing `{song.name}`. Volume is currently `{song.volume * 100}%` in `{ctx.voice_client.channel.mention}`**')
        else:
    ...
    async def _summon(self, ctx, *, channel: discord.VoiceChannel = None):
        ...
        if channel is None and not ctx.author.voice:
            raise MusicError('You are not in a voice channel or you did not state a voice channel for me to join')
    ...
    async def play(self, ctx, *, request: str):
        ...
        """
        msg = await ctx.send(":calling: | **Searching..**")
        ...
        if not ctx.music_state.is_playing():
            ...
            e = discord.Embed(description=f'Added {song} to the playlist in position #{ctx.music_state.playlist.qsize()}')
            await ctx.send(embed=e)

        await msg.edit(content=":cd: | **Finished searching**")
        time.sleep(1)
        await msg.delete()
    ...
    async def pause(self, ctx):
        ...
        if ctx.voice_client:
            ...
            await ctx.send(":pause_button: | **Paused playback.**")
    ...
    async def volume(self, ctx, volume: int = None):
        ...
        if volume < 0 or volume > 100:
            raise MusicError('**:mute: | Volume can only be between 0 and 100.**')
        ctx.music_state.volume = volume / 100
        e = discord.Embed(description="**:speaker: | Volume has been set to {0}.**".format(volume))
        await ctx.send(embed=e)
    ...
    async def clear(self, ctx):
        ...
        e = discord.Embed(description="**:stop_button: | Cleared playlist.**")
        await ctx.send(embed=e)
    ...
    async def skip(self, ctx):
        ...
        """
        if not ctx.music_state.is_playing():
            raise MusicError('No songs are playing')

        if ctx.author.id in ctx.music_state.skips:
            raise MusicError(f'{ctx.author.mention}, you already voted to skip this song')
    ...
    async def forceskip(self, ctx):
        ctx.music_state.skips.clear()
        ctx.voice_client.stop()
        await ctx.send("Force skipped current song")
    ...
    async def minskips(self, ctx, number: int):
        ...
        await ctx.send("Set minimum skips to {}".format(number))
